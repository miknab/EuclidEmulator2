...
cdef class PyCosmology:
     ...
     def __init__(self, double Omega_b , double Omega_m , double Sum_m_nu , double n_s , double h , double w_0 , double w_a , double A_s ):
        ...
        self._inst =new Cosmology((<double>Omega_b), (<double>Omega_m), (<double>Sum_m_nu), (<double>n_s), (<double>h), (<double>w_0), (<double>w_a), (<double>A_s))
...
cdef class PyEuclidEmulator:
     ...
     def compute_nlc(self,PyCosmology csm, redshift, n_redshift):
          self.ee2.compute_nlc((<Cosmology *> csm._inst)[0], redshift, n_redshift)
...
"""
files from ee2_input.py

EuclidEmulator2 submodule containing functions related to argument parsing.
"""
...
def check_param_range(par_dict): #, csm_index=0): #Only one cosmology for now
    ...
    if om_b_not_in_range:
        raise ValueError("Parameter range violation: \
                          Omega_b is set to %f, but should be in the interval \
                          [0.04, 0.06]."
                         %(par_dict['Omega_b'] ))

    if om_m_not_in_range:
        raise ValueError("Parameter range violation: \
                          Omega_m is set to %f, but should be in the interval \
                          [0.24, 0.40]."
                         %(par_dict['Omega_m']))

    if m_nu_not_in_range:
        raise ValueError("Parameter range violation: \
                          m_ncdm is set to %f, but should be in the interval \
                          [0.00, 0.15]."
                         %(par_dict['m_ncdm']))
    ...
    if w_a_not_in_range:
        raise ValueError("Parameter range violation: \
                          w_a is set to %f, but should be in the interval \
                          [-0.7,  0.5]."
                         %( par_dict['wa_fld']))

    if A_s_not_in_range:
        raise ValueError("Parameter range violation: \
                          A_s is set to %f, but should be in the interval \
                          [1.7e-9, 2.5e-9]."
                         %( par_dict['A_s']))
...
def convert_to_emu(class_pars_dict):
    ...
    if 'h' in class_pars_dict:
        ...
        print("Missing parameter h. Can't proceed.")
        sys.exit()
    ...
    if 'm_ncdm' in class_pars_dict:
        ...
        print("Missing parameter m_ncdm. Can't proceed.")
        sys.exit()


    # Should give either Omega_m or Omega_cdm
    # Omega_m currently prefered as otherwise won't include neutrinos
    if 'Omega_m' in class_pars_dict:
        ...
        print("Missing parameter Omega_m or Omega_cdm. Can't proceed.")
        sys.exit()
    ...
    if 'n_s' in class_pars_dict:
        ...
        print("Missing parameter n_s. Can't proceed.")
        sys.exit()
    ...
    if 'A_s' in class_pars_dict:
        ...
        print("Missing parameter A_s or ln10^{10}A_s. Can't proceed.")
        sys.exit()
...
def get_boost(cosmo_par_in,redshifts,custom_kvec=None):
    ...
    #Two options for checking redshifts are in range: either warn but do nothing (first), or throw exception (second)
    '''
    if (redshifts>10).any() or (redshifts<0).any():
        print('EuclidEmulator2 accepts only redshifts in the interval [0.0, 10.0]')
        print('All requested redshifts above 10 will result in the same (incorrect) boost as for z=10')
    '''
    for z in redshifts:
        assert z <= 10.0, "EuclidEmulator2 allows only redshifts z <= 10.0.\n"
    ...
    boost=10**np.reshape(ee2.Bvec[0:len(redshifts)],(len(redshifts),len(k)))

    #New part to extrapolate
    kvals = k
    k_shape = kvals.shape
    ...
    if len_redshifts > 1:
        ...
        for i in range(len_redshifts):
            ...
            if not(custom_kvec is None):
                ...
                bvals[i] = tmp.reshape(k_shape)
    else:
        tmp = boost
        if not(custom_kvec is None):
            bvals = 10.0**_CubicSpline(np.log10(kvals),
                                       np.log10(tmp.reshape(k_shape))
                                      )(np.log10(custom_k_within_range))
            ...
            if do_extrapolate_below:
                ...
                # the boost factor is unity by construction
                b_extrap = np.ones_like(custom_k_below)
                bvals = np.concatenate((b_extrap,bvals))

            if do_extrapolate_above:
                # We extrapolate by setting all b(k > k_max) to b(k_max)
                b_extrap = bvals[-1] * np.ones_like(custom_k_above)
                bvals = np.concatenate((bvals, b_extrap))
...
#    print("Computed boost for requested redshifts.")
